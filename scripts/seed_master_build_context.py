...
DEFAULT_CONFIG = ROOT / "athelon-app/docs/plans/contextual-seeding/industry-context-feature-map.json"
DEFAULT_STATE = ROOT / "athelon-app/docs/plans/contextual-seeding/industry-context-seed-state.json"
...
def parse_staged_new_docs(corpus_folder: str, index_name: str) -> list[str]:
...
def parse_index(index_path: Path) -> tuple[set[str], set[str]]:
    ...
    indexed_files = file_index_names | snapshot_names
...
def render_seeding_block(config: dict, seed_results: list[SeedResult]) -> str:
    section_title = config["output"]["section_title"]
    # Stable relative path from athelon-app/MASTER-BUILD-LIST.md to research corpus index.
    rel_index_str = str(Path("../research/industry-context-field-artifacts/INDEX.md"))
    ...
    for r in applied:
        ...
        context_link = f"[`INDEX.md` {('Application Context Mapping' if s['source_context_anchor'] == 'application-context-mapping' else 'File Index')}]({rel_index_str}#{s['source_context_anchor']})"
...
def upsert_master_seeding_block(master_path: Path, block: str, section_title: str) -> bool:
    ...
    if heading in original:
        ...
        insert_before = "### Auto-Discovered Master Features (Bug Hunter)"
...
def main() -> int:
    ...
    index_path = ROOT / corpus_folder / index_name
    ...
    if args.staged_docs.strip():
        ...
        staged_new_docs = parse_staged_new_docs(corpus_folder, index_name)

    indexed_files, domains = parse_index(index_path)
    ...
    blocked = [r for r in eligible if r.blocked_by_unindexed]
    ...
    block = render_seeding_block(config, seed_results)
