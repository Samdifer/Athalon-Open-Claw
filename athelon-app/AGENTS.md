...
This file applies to all work inside `athelon-app/`.
...
Run from `athelon-app/` after backend changes:
...
- `MASTER-BUILD-LIST.md` is the canonical, write-authoritative feature specification document.
...
```bash
npm run spec:sync:bug-hunter -- --report <json-report>
```

- Bug Hunter may mutate only:
  - `MASTER-BUILD-LIST.md` Registry A (`## Registry A — Master Features ...`) row upserts
