...
function runConvex(fnName, args) {
  ...
  const cmd = `npx convex run ${fnName} '${argJson}'`;
...
async function main() {
  ...
  if (!clerkUserId) {
    throw new Error(
      "Unable to resolve Clerk user ID for seeded persona. Set PLAYWRIGHT_SEEDED_USER_ID or provide a seeded auth state.",
    );
  }

  console.log(`Seeding scheduler stories for Clerk user: ${clerkUserId}`);
  const seedOutput = runConvex("seed:seedSchedulerStories", { clerkUserId });
  console.log(seedOutput);

  console.log("Running legacy schedule backfill for seeded stories...");
  const backfillOutput = runConvex(
    "schedulerPlanning:backfillLegacyScheduleAssignmentsSystem",
    {
      clerkUserId,
      includeClosed: false,
      createFallbackBayIfMissing: true,
      dryRun: false,
    },
  );
