...
export function RoleGuard({ allowedRoles, children }: RoleGuardProps) {
  const { isLoaded, needsBootstrap, awaitingProfileLink } = useCurrentOrg();
  ...
  if (awaitingProfileLink) {
    return (
      <div className="flex items-center justify-center min-h-[50vh] p-4">
        <Card className="max-w-md w-full">
          <CardHeader className="text-center">
            <div className="mx-auto mb-2 w-12 h-12 rounded-full bg-blue-500/10 flex items-center justify-center">
              <ShieldAlert className="w-6 h-6 text-blue-500" />
            </div>
            <CardTitle>Profile Link Pending</CardTitle>
            <CardDescription>
              An administrator must link your organization account to a personnel profile before you can access this page.
            </CardDescription>
          </CardHeader>
          <CardContent className="text-center">
            <Button asChild>
              <Link to="/onboarding">View Status</Link>
            </Button>
          </CardContent>
        </Card>
      </div>
